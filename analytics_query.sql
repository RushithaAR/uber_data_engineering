CREATE OR REPLACE TABLE `uber_data_engineering` AS (
SELECT 
f.trip_id,
f.VendorID,
...
`uber_data_engineering.fact_table` f
...
JOIN `uber_data_engineering.dropoff_location_dim` drop ON drop.dropoff_location_id=f.dropoff_location_id
JOIN `uber_data_engineering.payment_type_dim` pay ON pay.payment_type_id=f.payment_type_id)
;
